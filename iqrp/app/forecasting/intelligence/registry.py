...
def discover_engine_modules(root_package: str = "iqrp.app.forecasting") -> list[str]:
    ...
    for info in pkgutil.walk_packages(paths, root_package + "."):
        ...
        if leaf in {"registry", "mock"} or name.endswith(".models.mock"):
            found.append(name)
        elif leaf == "__init__" and ".models" in name:
            found.append(name)
...
def list_discovered_models(settings: DiscoveryConfig | IntelligenceSettings | None = None) -> list[DiscoveredModel]:
