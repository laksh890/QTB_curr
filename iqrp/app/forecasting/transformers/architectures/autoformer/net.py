...
class AutoformerNet(nn.Module if has_torch() else object):  # type: ignore[misc]
    def __init__(
        self,
        n_features: int,
        horizon: int,
        ...
        **kwargs: Any,
        ...
        self.input_emb = TransformerInputEmbedding(n_features, d_model, n_regimes=n_regimes, use_regime=use_regime, dropout=dropout)
        self.pos = build_positional("sinusoidal", d_model)
        self.encoder = TransformerEncoder(d_model, n_heads, num_layers, ffn_dim, dropout, attention_type="full")
    ...
    def forward(self, x: Any) -> Any:
        ...
        seasonal_r = reshape_forecast(seasonal_out, x.shape[0], self.horizon, task=self.task, n_classes=self.n_classes, n_quantiles=self.n_quantiles, dist=self.dist)
