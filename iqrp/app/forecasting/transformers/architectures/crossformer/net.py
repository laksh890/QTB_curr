...
class CrossformerNet(nn.Module if has_torch() else object):  # type: ignore[misc]
    def __init__(
        self,
        n_features: int,
        horizon: int,
        ...
        **kwargs: Any,
        ...
        self.cross = CrossAssetAttention(d_model, n_heads, dropout)
    ...
    def encode(self, x: Any) -> Any:
        ...
        h = self.encoder(h)
        # treat features as pseudo-assets: (B,F,T,D) via split dims
        b, t, d = h.shape
        # reshape as single asset
        ha = h.unsqueeze(1)
        return self.cross(ha).squeeze(1)
