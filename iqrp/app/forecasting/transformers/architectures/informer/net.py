"""Informer ProbSparse Transformer network."""
...
class InformerNet(nn.Module if has_torch() else object):  # type: ignore[misc]
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.pos = build_positional(positional if positional != "rotary" else "sinusoidal", d_model)
        self.encoder = TransformerEncoder(
            d_model, n_heads, num_layers, ffn_dim, dropout, attention_type="sparse"
        )
    ...
    def encode(self, x: Any) -> Any:
        h = self.input_emb(x)
        if hasattr(self.pos, "emb") or hasattr(self.pos, "pe"):
            h = self.pos(h)
