"""Mixture-of-Experts Transformer network."""
...
class MoETransformerNet(nn.Module if has_torch() else object):  # type: ignore[misc]
    def __init__(
        self,
        n_features: int,
        horizon: int,
        ...
        **kwargs: Any,
        ...
        self.moe = MoEGating(d_model, n_experts=max(n_regimes, 2), ffn_dim=ffn_dim, dropout=dropout)
        self.head = forecast_head(d_model, horizon, task=task, n_classes=n_classes, n_quantiles=n_quantiles, dist=dist)

    def encode(self, x: Any) -> Any:
        h = self.encoder(self.pos(self.input_emb(x)))
        return self.moe(h)
