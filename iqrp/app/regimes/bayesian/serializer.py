"""Serialize / deserialize Bayesian regime-switching models."""
...
class BayesianSerializer:
    def save(self, model: Any, path: Path) -> Path:
        ...
        for key in ("mean_transition", "mean_initial", "mean_means", "mean_covars"):
            if key in algo and algo[key] is not None:
                arrays[key] = np.asarray(algo[key], dtype=np.float64)
        post = algo.get("posterior") or {}
        if post.get("draws"):
            # store a compact last-draw snapshot for resume
            last = post["draws"][-1]
            for key in ("transition", "initial", "means", "covars"):
                if key in last:
                    arrays[f"last_{key}"] = np.asarray(last[key], dtype=np.float64)
        if arrays:
