"""Serialize / deserialize fitted HMM models."""
...
class HMMSerializer:
    def save(self, model: Any, path: Path) -> Path:
        ...
        emis = algo.get("emissions") or {}
        for key in ("means", "covars", "probs"):
            if key in emis and emis[key] is not None:
                arrays[f"emis_{key}"] = np.asarray(emis[key], dtype=np.float64)
        if arrays:
    ...
    def load(self, path: Path, *, model_cls: type[M]) -> M:
        ...
        if sidecar.exists():
            ...
            emis = dict(algo.get("emissions") or {})
            for key in data.files:
                if key.startswith("emis_"):
                    emis[key.removeprefix("emis_")] = data[key].tolist()
                else:
