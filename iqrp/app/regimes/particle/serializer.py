"""Serialize / deserialize particle filter models."""
...
class ParticleSerializer:
    def save(self, model: Any, path: Path) -> Path:
        ...
        for key in ("means", "covs", "ess", "resampled", "smooth_means", "smooth_covs"):
            if key in algo and algo[key] is not None:
                arrays[key] = np.asarray(algo[key], dtype=np.float64)
        if "cloud_states" in algo and algo["cloud_states"] is not None:
            arrays["cloud_states"] = np.asarray(algo["cloud_states"], dtype=np.float64)
        if "cloud_log_weights" in algo and algo["cloud_log_weights"] is not None:
            arrays["cloud_log_weights"] = np.asarray(algo["cloud_log_weights"], dtype=np.float64)
        if arrays:
